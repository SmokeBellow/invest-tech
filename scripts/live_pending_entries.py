...
источников, никогда не смешиваются:
  ...
  1. ТРИГГЕР — считается ИСКЛЮЧИТЕЛЬНО по data/*.csv в том виде, в каком
     их зафиксировал вчерашний ночной прогон (уже закрытый бар). Этот
     скрипт НИКОГДА не дозаписывает и не подмешивает сегодняшние данные
     в data/*.csv перед расчётом триггера — сигнал полностью определён
     ДО открытия сегодняшней сессии, ровно как на настоящем счёте.
